package com.yuri.yeaicodegenplatform.langgraph4j;
...
public class SimpleStatefulWorkflowApp {
     ...
     * @return
     */
    static AsyncNodeAction<MessagesState<String>> makeStatefulNode(String modeName, String message) {
        return node_async(state -> {
            WorkflowContext context = WorkflowContext.getContext(state);
            log.info("执行节点：{} - {}", modeName, message);
            // 只记录当前步骤，不做具体的状态流转
            if (context != null) {
                context.setCurrentStep(modeName);
            }
